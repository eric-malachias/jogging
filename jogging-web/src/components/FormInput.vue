<template>
    <div class="form-input">
        ...
        <alert v-if="isInvalid()" type="danger" :dismissable="false" :content="t(`error.${getError()}`)"></alert>
    </div>
</template>
...
export default {
    props: ['name', 'maxlength', 'value', 'error', 'label', 'type', 'only'],
    components: {
        Alert
    },
    data () {
    ...
    methods: {
        ...
        getError () {
            return this.error.body[this.name][0]
        },
        hitReturn () {
            this.$emit('return')
        },
